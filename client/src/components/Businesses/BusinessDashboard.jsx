  ...
  getDeliveryTeam = async () => {
    ...
    try {
      const { data } = await axios.post(
        'http://localhost:3000/api/business/getDeliveryTeam',
        body
      );
      this.setState({
        myDeliveryTeam: data,
  ...
  async currentOrders() {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/cart/grabBizOrders/${this.state.bizId}`
      );
      ...
      <div className="dashboard">
        ...
        <div className="dashboardOrders">
          ...
          {this.state.orders && this.state.orders.length ? (
            this.state.orders.map(item => <EachOrder order={item} />)
          ) : (
            <div />
          )}
        </div>
        ...
        <div className="dashboarDeliveryTeam">
          <button className="addDriver" onClick={this.addDeliveryPerson}>
            <i className="fas fa-plus icon" />
          </button>
          {this.state.myDeliveryTeam.length
            ? this.state.myDeliveryTeam.map(driver => (
