...
class OpenPools extends Component {
  constructor() {
    ...
    this.state = {
      discount: ['no discount yet', 'free delivery', 0.05, 0.1, 0.15],
    };
    this.calcDiscount = this.calcDiscount.bind(this);
    this.nextDiscount = this.nextDiscount.bind(this);
    this.peopleNeededForNextDiscount = this.peopleNeededForNextDiscount.bind(
      this
    );
  }
  ...
  async grabPools() {
    const { data } = await axios.get(
      `http://localhost:3000/api/pool/grabAllPools`
    );
    this.setState({
      openPools: data,
    });
        ...
        {openPools && openPools.length
          ? openPools.map(pool => (
              <div className="usersPool" key={pool.bizName}>
                ...
                <br />
                <div className="poolDiscount">{`${this.calcDiscount(
                  pool.count
                )}`}</div>
                <label>next discount</label>
                <br />
                <div className="poolNextDiscount">
                  {this.nextDiscount(pool.count)}
                </div>
                <label>delivery time:</label>
                <br />
                <div className="poolDeliveryTime">{`${
                  moment(pool.timer)
                    .countdown()
                    .toString()
                    .split('and')[0]
                }`}</div>
                <label>next discount in:</label>
                <br />
                <div className="poolPeopleNeeded">
                  {this.peopleNeededForNextDiscount(pool.count)} users
                </div>
